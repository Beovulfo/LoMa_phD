...
!-----------------------------------------------------------------------
      ...
      a2n(j) =  0d0          !0d0         !
      a1n(j) =  0d0          !0d0         !
      a0 (j) =  -11d0/3d0/h  !-10d0/3d0/h !
      a1p(j) =  -9d0/h       !-3d0/h      !
      a2p(j) =  -a1p(j)      !6d0/h       !
      a3p(j) =  -a0(j)       !1d0/3d0/h   !
...
!   -------  this is banbks7 adapted to work on the transpose ---
      ...
      fwk(1)= prem1(1,1)                    ! ---- this is the delta 
      fwk(2)= -prem1(1,2)*prem1(2,1)*fwk(1) ! everything else has no rhs
      fwk(3)= -prem1(1,3)*(prem1(2,2)*fwk(2)+prem1(3,1)*fwk(1))

      do j=4,my
         fwk(j) = -prem1(1,j)*(prem1(2,j-1)*fwk(j-1)+
     .      prem1(3,j-2)*fwk(j-2) + prem1(4,j-3)*fwk(j-3))
      enddo

!     back substitution

      fwk(my-1) = fwk(my-1)-prem1(5,my-1)*fwk(my)
      fwk(my-2) = fwk(my-2)-prem1(5,my-2)*fwk(my-1)-
     .            prem1(6,my-2)*fwk(my)
      do j=my-3,1,-1
        fwk(j) = fwk(j)-prem1(5,j)*fwk(j+1)-prem1(6,j)*fwk(j+2) - 
     $           prem1(7,j)*fwk(j+3)
      enddo
...
!c
!cprepares cfdiff pieces for using cfdiff subroutines and laps later
!c
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
