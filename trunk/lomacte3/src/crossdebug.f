...
!     !debug
      complex*8 ::  deb1(0:my-1,0:mgalz-1,pb:pe),
     .          deb2(0:my-1,0:mgalz-1,pb:pe)
...
c ========================================================================
      do 30 istep=1,nstep
         ...
         IF (mod(istep-1,nimag) .eq. 0 .and. istep.ne.1) then
            ...
            enddo
!------DEBUG
            !saving LAPPSI and rho
            call escru(vor,phi,deb2,deb1,u00,w00,v00,myid)
            if (myid.eq.0) then
               write(*,*) "Debug is on!"
               write(*,*) "istep=",istep,"Dt=",Deltat
            endif
!----------
...
c  ---------- back to the yz planes an fft 
      ...
      !debug
      call chln2pl(deb1,deb1,chwk,myid) !rho
...
!transpose back
      ...
      !deb1
      call localzy2yz(deb1,deb1,chwk)
!------------------------------------------------------!
...
c--------------------- back to F-P-P  
         ...
         !debug
         call fourx(deb1wk,up1wk8,-1) !deb1
