...
!--------------  allocates spectra and buffers
      nspsize   = (2*nspec+1)*(nz1+1)*12*(pe-pb+1)
...
!CHECK/WARNING for IMAGINARY PART ON MODES (0,kz)
      if (myid.eq.0) then
         if (planex.eq.1) then
            write(*,*) "Checking Imaginary part of MODES (0 , kz)..."
            do k=1,mz
               do j=2,2*my,2 !Imaginary parts are even
                   if (vor(j,k).ne.zero) then
                       flag = flag + 1
                       vor(j,k) = 0.0
                   endif

                   if (phi(j,k).ne.zero) then
                       flag = flag + 1
                       phi(j,k) = 0.0
                   endif

                   if (psi(j,k).ne.zero) then
                       flag = flag + 1
                       psi(j,k) = 0.0
                   endif

                   if (scal(j,k).ne.zero) then
                       flag = flag + 1
                       scal(j,k) = 0.0
                   endif
               enddo
            enddo
            if (flag.eq.0) then
               write(*,*) 'All modes (0,kz) set to zero...GGWP'
            else
               write(*,*) 'FOUND & CORRECTD', flag,
     .                    'exceptions of modes (0,kz)'
            endif
         endif
      endif
