...
!RK appropiate to use with PSI updating
      ...
      parameter (xi   =(/        0d0,-17d0/60d0, -5d0/12d0 /)) !aaf 
      parameter (gama= (/ 8d0/15d0,   5d0/12d0,   3d0/4d0 /))
      parameter (alpha=(/ 0d0, -3d0/40d0, 85d0/6d0/))
      parameter (beta =(/ 8d0/15d0, 5d0/24d0,   29d0/2d0 /))
...
!save buffer with RHS of Scal in order to use on tauij2
...
!              call rkstepexp(scal(0,0,i),rhst(0,0,i), mz,i,dtgamma)
          ...
          if (rkstep.eq.1) then
          !for first substep we need the RHST only
             do i=pb,pe
                do k=0,mz1
                   do j=0,my1
                     chwk2(j,k,i)= rhst(j,k,i)
                   enddo
                enddo
             enddo
          else
             do i=pb,pe
                do k=0,mz1
                   do j=0,my1
                     chwk2(j,k,i)= chwk2(j,k,i)+rhst(j,k,i)*dtgamma
                   enddo
                enddo
             enddo
          endif
...
!   ten13: -
!   ten23: -
!   drho : rho(n+1) - rho (n) (first substep= RHS(rho))  
!   rhst : RHS(T) -->RIP buffer, we keep it until the end of substep
!*******************************************************************!
...
!        1.1) Integrate drho00
              ...
              if (rkstep.eq.1) then
                 ...
                 do j=0,my1
                     drho00(j)=real(drho(j,0,1))/dtbeta-alpha(rkstep)/
     .                          beta(rkstep)*drho00(j)
                 enddo 
              endif
              !    call inty8(drho00,rf0v)
              !CUMTRAPZ:
              rf0v(0) = 0d0
              do j=0,my1-1
                 rf0v(j+1) = rf0v(j)+0.5*(y(j+2)-y(j+1))*
     .                (drho00(j+1)+drho00(j))
              enddo
...
!----------------------------------------------------------------------
!2) Solve other modes...with same scheme
!New variable:
!   2.1)solve Lapvdv for -drho
         do i=pb,pe
            do k=0,mz1
              do j=0,my1
                  ten23(j,k,i) =-drho(j,k,i)/dtbeta 
              enddo
            enddo
         enddo
         do i=pb,pe
            do k=0,mz1
               k1 = icx(k)
               rK = bet2(k1)+alp2(i-1)
               call Lapvdvhom(ten23(0,k,i),ten23(0,k,i),chwk2(0,k,i),rK)
            enddo
         enddo
!   2.1)Update psi(n+1)
         if (rkstep.eq.1) then
             ...
             do i=pb,pe
                do k=0,mz1
                  do j=0,my1
                     psi(j,k,i)=ten23(j,k,i)-alpha(rkstep)
     .                      /beta(rkstep)*psi(j,k,i)!RK3 implicit
                  enddo
                enddo
             enddo 
          endif
...
! Dt_visc=1/(2 nu)*Dy_min^2
      hyy=2/Re*(max(hyy,max(hxalp/2.0,hzbet/2.0)))**2.0
...
! ----------------------------------------------------------------------!
        if (rkstep.eq.1) then
            do i=0,mgalx-1
            !up1wk = RHST(rho) = -1/T^2 * RHS(T)
                up1wk(i)=-up2wk8(i)/(up1wk8(i)**2.0) 
           enddo
        else
