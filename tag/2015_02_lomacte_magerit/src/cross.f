...
c ========================================================================
      ...
      do 30 istep=1,nstep
          !write(*,*) "myid=",myid,"Re=",Re,"rhocrit=",rhocrit
         if (myid.eq.0) then
...
!        if (myid.eq.0) then
!328        format(3(d22.14))
!           if (rkstep.eq.1.and.mod(istep,10).eq.0) then
!              write(ext1,'(i3.3)') istep
!!        write(ext2,'(i3.3)') rkstep
!              fdebug='rhou'//ext1//'.deb' 
!        !fdebug='rhou'//ext1//ext2//'.deb' 
!               open(unit=40,file=fdebug,status="unknown")
!                      write(40,328) (y(j+1), real(mxwk(j,1,1)), 
!     .                                       real(mxwk(j,0,2)),
!     .                              j=0,my1) !write mode 0,1 and 1,0
!               close(40)
!           endif
!        endif
!!!----------------
...
!        1.1) Integrate drho00
     ...
     .        !        (drho00(j+1)+drho00(j))
              ...
              do j=0,my1
                  ...
                  !rf0v(j)=(rf0v(j)-rf0v(0))-rhobot*trapz/(rhotop+rhobot)
...
!***************************************************************************!
 ...
 10    continue  !!! End of R-K sub-step
       ...
       if (ihist.eq.1.and.istati.eq.1) then
            ...
            if (myid.eq.0.and.istep.ne.1) then
                 writetimer = writetimer + MPI_WTIME()
                 enerdis =0.0
...
!initialization
      !aaf Peclet
      peclet=0.7d0*re   !Pr = 0.5 for testing
