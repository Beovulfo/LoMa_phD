%****************************************************************************%
% Matlab script to create starting file for LISOuc3m			     %
% Antonio Almagro							     %
% May 2013								     %
...
time=0;
Ree= Reb;%Reb because our profile is for ub=1.
...
[v,phi,vor,ee1,vecty]=ossq_for_toni(kx1,kz1,Reb,mye-2);
...
kz2=1;
%Calculation for each desired mode
[v,phi,vor,ee2,vecty]=ossq_for_toni(kx2,kz2,Reb,mye-2);
...
linez1 = round(kz1/bete) + 1
planex1 = round(kx1/alpe)+1
k1=linez1 %kz=0 means k=1 (first plane) 
%Z lines goes from 0 (first) to mze-1 (last) [FORTRAN], 
% but matlab goes from 1 to mze
	for j=1:mye
		wk1(1,2*j-1,k1) =real(vor1(j));
		wk1(1,2*j  ,k1) =imag(vor1(j));
       	        wk1(2,2*j-1,k1) =real(phi1(j));
 		wk1(2,2*j,  k1) =imag(phi1(j));
	end

wk2=zeros(2,2*mye,mze);
linez2 = round(kz2/bete) + 1
planex2 = round(kx2/alpe)+1
k2=linez2 %kz=0 means k=1 (first plane) 
%Z lines goes from 0 (first) to mze-1 (last) [FORTRAN], 
% but matlab goes from 1 to mze
	for j=1:mye
		wk2(1,2*j-1,k2) =real(vor2(j));
		wk2(1,2*j  ,k2) =imag(vor2(j));
       	        wk2(2,2*j-1,k2) =real(phi2(j));
 		wk2(2,2*j,  k2) =imag(phi2(j));
	end
...
finp=fopen('../bin/finp.dat','w','l'); %open/create file tab_var.dat
