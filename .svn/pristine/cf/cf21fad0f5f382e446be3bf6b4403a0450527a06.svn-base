...
! Creating y window FILTER (gaussian type)
      if (s.eq.1) then
        ...
        write(*,*) "applying shifting to gaussian filter
     .              for s=",s 
      !a fair approximation for the shifting on the
      !rms profiles is s/2 (for s=8-->4
        do j =1,my
            ywin(j) = exp(-((y(j)-s/2.0)/swin)**2)
        enddo
      endif
...
!------------------
      ...
      !URMS at mid
      !find u=0
      !change of sign on umean
      j=1
      do while  (Um0(j)*Um0(j+1).gt.0)
         j = j+1
      enddo
      write(*,*) "plane at u=0: ",j 
      urms = 0.0
      vrms = 0.0
      wrms = 0.0
      do i=1,nplanes
      ...
      ! Adding 00 modes
      do j=1,my
          !rho0 = 1, rho distribution simetric, T non-simetric
          !rho00(j) =(1d0 + (s-1d0)/(s+1d0)*tanh(-y(j)/(2d0*dm0)))
          !rho00(j) =(1d0 + (s-1d0)/(s+1d0)*tanh(-y(j)/(2d0*dm0)))
          rho00(j) =rho0(j) !read from file
          rhofou(j,1,1) = rho0(j)
          !without perturbation on rho/T we only need to specify 00 mode
          ufou(j,1,1) = Um0(j)!read from input
...
!check if s.neq.1
       if (s.ne.1d0) then
          !transfrom to phys in order to find rhou
!!================================================================!
!!going to phys 
!
...
!---------------------------------------------------------------!
      ! Save mode 00
      write(*,*) 'Case of density ratio different to one'
      do j = 1, my
          rhou00(j) = ufou(j,1,1) 
          rhov00(j) = vfou(j,1,1)
          rhow00(j) = 0d0
...
! SCAL
      ...
      if (s.eq.1) then
         write(*,*) 'Case of density ratio one'
         do j=1,my
           T00(j)   = Tm0(j) 
         enddo
      !s can be used to multiply scalar max
         do j=1,my
            !now s = 1.0,so max of scalar is 1
            scal(j,1,1)=T00(j)
         enddo
      ! Save mode 00
         do j = 1, my
...
!      !READ MESH and PROFILES
       write(*,*) "Reading mesh..."
       open(iinp,file=filinp,status='old')
       read(iinp,*) (y(j),Um0(j),rho0(j),rum0(j),Tm0(j),Vm0(j),
     .               j=1,my)
       rewind(iinp)
       close(iinp)
