...
Cy=0.05; %Controls variation introduced as function of y
%pert=0.001;%Controls variations with respect modes 
%C2=0.000002;%C2 for omega y
C2=C/100.;%C2 for omega y 2 orders smaller than C1
C2= 0;
...
%ymax=200/2; %half heigth of the box
%mxe and mze must be some multiple of 64*3/2
%mye=513 mlpantano13
%mye=513 %mlpantano13y2
...
%y=create_mesh1D(ymax,'manl',1 ,mye); 
y=create_mesh1D(ymax,'file',1 ,mye); %read mesh.txt file
...
%size of Dy
Dy=min(hy)
figure(1); plot(y(1:my-1),diff(y),'b*')
%============================================================%
%ZERO MODES
%by Rogers&Moser
...
%DT = 1.0; %COMPRESSIBLE
DT= DT/2;
T00=Tmean+DT*tanh(-y./(2*dm0));

%tanh
u00=u0./T00.*tanh(-y./(2*dm0));
%dm0 set to 1.
   ...
   if (nxvector(i)<=modecut)
      ...
      for k=1:mze
         if (abs(nzvector(k))<1) %eliminate 2D modes
       		...
       		wk1(3,indreal,k)  =zeros(my,1); %PSI
       		wk1(3,indreal+1,k)=zeros(my,1); %PSI imag
       		wk1(4,indreal,k)  =zeros(my,1); %T 
       		wk1(4,indreal+1,k)=zeros(my,1); %T imag
      	 else
         %fill the wk1
      		contador=contador+1;
...
%Writing
   ...
   % plot(y,wk1(2,indreal,41));
    %Add perturb
    wk1(:) = (pert*(2*rand(1)-1)+1).*wk1(:);
    %ADD 00 mode of Temperature
    if (i==1) %plane0
       wk1(4,indreal,1) = wk1(4,indreal,1)+T00; 
    end
