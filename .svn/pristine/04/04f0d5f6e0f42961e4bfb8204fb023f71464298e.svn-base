...
!------------------
      ...
      do i = 1,nplanes
         do k = 1,mz
            do j=1,my
                 ufou(j,k,i) = ufou(j,k,i)*euutarget/urms
                 vfou(j,k,i) = vfou(j,k,i)*evvtarget/vrms
                 wfou(j,k,i) = wfou(j,k,i)*ewwtarget/wrms
            enddo
         enddo
      enddo
...
326   format(2(d22.14))
      ...
      close(11)
!Save 00 modes of rhov and rhow
      do j = 1, my
...
! SCAL
      ...
      ! Save mode 00
         ...
         do j = 1, my
           rhou00(j) = rho00(j)*u0*tanh(-y(j)/(2d0*dm0)) !rhou00 with no aliasing errors
           !rhou00(j) = rho00(j)*u0*tanh(-y(j)/(2d0*dm0)) !rhou00 with no aliasing errors
           rhov00(j) = 0d0
           rhow00(j) = 0d0
         enddo
         do j=1,my
            scal(j,1,1)=1d0/rho00(j)
         enddo
      endif
