...
!FLAG.................................................................
!            do j=0,my1
!               write(*,*) y(j),u00(j),w00(j),rf0u(j),rf0w(j),fmap(j+1)
!            enddo
!......................................................................
         endif
...
!aaf vorwk/phiwk already keeps all the RHS we need to solve visc
...
!        call mpi_barrier(MPI_COMM_WORLD,ierr) 
     ...
     &           dalre,dtgamma,dalbe,dtri,dtxi,rkstep,2)
...
! massu1 will keep the momentum thickness
          dm=0d0
          do j=1,my
             dm=dm+0.25*(hy(j)*(1d0-(um(j))**2))
          enddo
!------------------------------------------------!
      endif
!      allocate(wk2(0:my-1,0:mgalz-1,pb:pe))
...
! preparation of rhs for next step
      ...
      endsubroutine
!-------------------------------------------------------------------------------!
! MODE 00 FOR W - need to be checked in depth 
! Similar to rku00 but with different BC's
      subroutine rkw00(f,rhs,nl,rkn1,dalre,dtgamma,dalbe,
     .                dtri,dtxi,rkstep)
      implicit none
      include "ctes3D"
...
!--------- ----------- BC functions----------------------------
      ...
      real*8 temp
! ----------------------------------------------------------
! df will be changed later during a call
      do j=1,my      !--- to real*8, fitf
         df(j)=f(j)
      enddo

!BC constants
!     Cvtop*w00(my)+Vdyvtop*w00(my)=BC_top
!     Cvbot*w00(1)+Vdyvbot*w00(1)=BC_bot
!     
!     TOP
!     -----------
      Cvtop=1d0
...
!         call deryyr(df,nl,rkn1,dalre,dtgamma)
     ...
     $            dt22(4,2)*df(3) + dt22(5,2)*df(4)
        do j=3,my-2
           ...
           fwk(j) = temp
        enddo
        fwk(my-1)=dt22(1,my-1)*df(my-3) + dt22(2,my-1)*df(my-2)+ 
     $            dt22(3,my-1)*df(my-1) + dt22(4,my-1)*df(my  )
        fwk(my)=  dt22(1,my  )*df(my-2) + dt22(2,my  )*df(my-1)+ 
     $            dt22(3,my  )*df(my)
! solve second derivative of u(rkstep) -> fwk
! fwk=d2y2(u00)
...
! preparation of rhs for next step
      ...
      enddo
!FLAG
!      write(*,*),"AA(1:2,1:2)", AA(1,1),AA(1,2),AA(2,1),AA(2,2)
!      write(*,*),"coef(1),coef(2)", coef(1),coef(2)
