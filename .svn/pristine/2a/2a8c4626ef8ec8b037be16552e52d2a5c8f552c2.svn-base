...
!                                                                       !
!***********************************************************************!
      subroutine Lapvdv2(phi,v,dvdy,rK)
...
! ----------------------- workspace ---------------------------
      ...
      real(8) :: Alin,Blin !linear part of Psi00:
                            !Psi00=Psi00p+Alin*y+B
...
!aaf ------------MODE00 (RK=0)--------------------!
! CAUTION...
      !if mode 00 solve only particular,
      if (rK.eq.0) then
...
! resolve v (real coefficients):
      ...
      !not setting vpr=0 on Boundaries(free!)
      call calcrhsfree(v1,vpr)
      call calcrhsfree(v2,vpi) 
...
!Set BC for MODE00:
      !dPsi00/dy(Ly)=-dPsi00/dy(-Ly)
      !Alin=-1/2*(psi00'(-Ly)+dpsi00'(Ly))
       !Alin=-0.5d0*(dvpr(1)+dvpr(my))
       Alin=0.0
      !Setting Psi00 = 0 @ y=Ly (for instance..)
      !Blin=-psi00p(Ly)-Alin*Ly
       !Blin=-vpr(my)-Alin*y(my)
       Blin=zero
        do j=1,my
          !v   (1,j) =  vpr(j) + Alin*y(j)+ Blin
          v   (1,j) =  vpr(j) + Alin*y(j)
          dvdy(1,j) =  dvpr(j)+ Alin
        enddo
!imaginary part set to zero...
      !dPsi00/dy(Ly)=-dPsi00/dy(-Ly)
      !Alin=-1/2*(psi00'(-Ly)+dpsi00'(Ly))
       !Alin=-0.5d0*(dvpi(1)+dvpi(my))
      !Setting Psi00 = 0 @ y=Ly (for instance..)
      !Blin=-psi00p(Ly)-Alin*Ly
       !Blin=-vpi(my)-Alin*y(my)
       !Blin=zero
        do j=1,my
          !v   (2,j) =  vpi(j) + Alin*y(j)+ Blin
          v   (2,j) =  vpi(j) + Alin*y(j) 
          dvdy(2,j) =  dvpi(j)+ Alin
        enddo

!!Write mode00 info
!        !write v
!        open(38,file='eta00.txt', status='unknown')
!        write(38,169) (y(j),v(1,j),j=1,my)
!        call flush(38)
!169     format(d22.14,X,d22.14)


       !exit from subrotuine:
       return
      endif
!---------------------------------------------------------------------!
!------end of 00 mode solver------------------------------------------!
!---------------------------------------------------------------------!
!      if (rK.eq.0) then
!        do j=1,my
!          v   (1,j) =  zero
...
! ---- for rest of modes solve system as usual........................
!-----------------------------------------------------------------------
