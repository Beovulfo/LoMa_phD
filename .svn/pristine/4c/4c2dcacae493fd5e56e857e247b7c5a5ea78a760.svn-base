...
!         !Calculate u,v,w
         ...
         do i=0,mgalx-1
              ...
              !Save DT = DH + dTdZ*DZ;
              dtwk8(i) = dtxi*(RHSHp(i,j)+dTdZ(znwk8(i))*
     .                   *RHSZp(i,j))
         enddo
...
!         !Now we have RHSH,RHSH,and dTdxi instead of dHdxi
         ...
         !ten13wk8 --> dTdy
         !ten23wk8 --> dTdz
        !Compute first part of lap(psi) update
         do i=0,mgalx-1
...
!---------------------------------------------------
         ...
         do i=0,mgalx-1
              ...
              !Save DT = DH + dTdZ*DZ;
              dtwk8(i) = dtgamma*(RHSHp(i,j)+dTdZ(znwk8(i))*
     .                   *RHSZp(i,j))+dtwk8(i)
         enddo
         !Record value
         do i=0,mgalx-1
        ...
        !Compute Drho
         ...
         !12/07/2017: Eliminate round-off error by saving
         !            DT = DH+dTdZ*DZ
           drhowk(i)=tnwk8(i)+ 
         !Option 1: use pseudo increment of T)
     .     dtwk8(i)/(rhstwk8(i)*tnextwk8(i)*dtbeta)
!        !Option 0:use DT (not 1/T-1/T)
!     .     (rhstwk8(i)-tnextwk8(i))/(rhstwk8(i)*tnextwk8(i)*dtbeta)
!!
!Before May12th,2017:     
