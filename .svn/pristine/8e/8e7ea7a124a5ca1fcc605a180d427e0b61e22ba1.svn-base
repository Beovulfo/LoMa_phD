...
!Save densities at boundaries
      if (myid.eq.0) then
           rhobot =1.0/ real(scal(0,0,1)) !mode 00,bot
           rhotop = 1.0/real(scal(my-1,0,1)) !mode 00,top
      endif
c ========================================================================
...
!initialization
      !aaf Peclet
      peclet=0.7*re   !Pr = 0.5 for testing
...
! obtain the equivalent y index (height of block lines).
         ...
         if (istati.eq.1 .and. rkstep.eq.1) then
            ...
            enddo
            do i=0,mgalx-1
               rhom(jj) = rhom(jj) +1.0/rhstwk8(i) !rho
            enddo
         endif
...
!         !Calculate rhs(T)
     ...
     .      (1d0/peclet)*rhstwk8(i)*ten22wk8(i)
