...
finp=fopen('../data/finpcte1025.dat','w','l'); %open/create file tab_var.dat
%Record 1 - counting bytes
rec1_real4=5+2*my2;%time is real 4
...
ntotr2=4*my2*mz %FOR EACH PLANE
rec2_size=ntotr2*4; %SAME SIZE FOR EACH PLANES
indreal=(2*(1:my)-1);
indreal2=(2*(1:my2)-1);
wk2=zeros(2,2*my2,mz);

for i=1:nplanes
    %reshape for each plane i
    wk1p=squeeze(reshape(wk1(i,:),2,2*my,mz));
    for k=1:mz
%scale and save vor 
      wk2(1,indreal2  ,k)=interp1(y,wk1p(1,indreal  ,k),y2,'linear',0);
      wk2(1,indreal2+1,k)=interp1(y,wk1p(1,indreal+1,k),y2,'linear',0);
%write PHI
      wk2(2,indreal2  ,k)=interp1(y,wk1p(2,indreal  ,k),y2,'linear',0);
      wk2(2,indreal2+1,k)=interp1(y,wk1p(2,indreal+1,k),y2,'linear',0);
