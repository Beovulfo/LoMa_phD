...
# 
	...
	if varpln[0:2]=='up':
		...
		for i in range(nmin,nmax+1,stride):
			...
			filename = HomeDir+jobname+'_'+'%3.3i'%i+'.'
			print 'Opening: ' + filename
    			[field[0],y,x,z] = readfieldij(filename+'o1'+pl,pert);
    			[field[1],y,x,z] = readfieldij(filename+'o2'+pl,pert);
    			[field[2],y,x,z] = readfieldij(filename+'o3'+pl,pert);

			filename2=filename+'vor'+pl
			if pert==1:
				print "Taking the mean"
				filename2=filename+'vorp'+pl
        		print 'Field shape:',np.shape(field)
			create_vtk_vector(x,y,z,field,'vor'+pl,filename2,binary)
