...
def RK3evolve(my,CFL,nstep,Tmean,deltaT,RKopt, param=0.0,param0=0.0,verb = False,time0=None, y=[], ru00=[], rv00=[], t00=[]):
	...
	if RKopt==2: #Update using N(rho) directly (NOT low-storage)
		...
		#Obtain timestep
			Dt = max(1./Dtvisc,max(rhov00*T00)/min(diff(y)),max(rhou00*T00)/min(diff(y)))
			Dt = CFL/Dt
