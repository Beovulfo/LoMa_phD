...
!--------------  allocates spectra and buffers
      ...
      nwkasize  = 7*nbuffsize + 8*my +3*nbuffsize!adding 3 scalar field
      allocate(sp(nspsize))
...
! get scalar field from file
          call getscal(scal,wk,myid)
!debug
!      write(*,*) 'SCAL(1),SCAL(my),myid',scal(1),scal(my),myid

!AAF need to grate some subroutine in order to read
! from file "filscainp" and save into scal(i)
      endif
...
!added by aaf
      ...
      iscal2   = iscal1   + nbuffsize
      iscal3   = iscal2   + nbuffsize
...
!***********************************************************************!
     ...
     .          ,scal1(0:my-1,0:mgalz-1,pb:pe)
     .          ,scal2(0:my-1,0:mgalz-1,pb:pe)
     .          ,scal3(0:my-1,0:mgalz-1,pb:pe)
...
!                   aaf 2014/01                                         !
!***********************************************************************!
      subroutine getscal(scal,wk1,myid)
...
!      ---------------  read from file and distribute
      ...
      if (myid.eq.master) then     
         ...
         open(iinp,file=filscainp,status='unknown',form='unformatted')
...
c         ------------  other modes,  master node -----
         ...
         enddo
!debug
!            write(*,*) 'MYID',myid,'scal(1,1,pb)',scal(1,1,pb)
!            write(*,*) 'MYID',myid,'scal(my-2,1,pb)',scal(my-2,1,pb)
