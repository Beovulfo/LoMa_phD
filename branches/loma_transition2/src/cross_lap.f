           ...
           do i=pb,pe
             ...
             if (i.eq.1) then
                do j=0,my1
                  v1(j)=mywk(j,1,i)
                enddo
                  !save derivative on y @v2
                  call deryr2(v1,v2,1)
                  call deryr2(v2,v3,1)
                  call deryr2(v3,v2,1)
                  !now v2 has second third derivatve on v1
                  
                  !compute directly second derivative
                  call deryyr2(v1,v3,1) 
                  call deryr2 (v3,v1,1) 
             !write the different between them
                do j=0,my1
                   v2(j)=v2(j)-v1(j)
                enddo
                write(*,*) "d3v(1,1)-d(d2(v(1,1)))"
                write(*,*) (v2(j),j=0,my1) 
             endif
...
! Solving eq system 
         ...
         enddo
!
!------------ACHTUNG- INCOMPRESSIBLE--------------------------------!
!For debugging porposes we use laplacian instead of Mx, My Mz
!combinations
!But mode 00 still has derivatives..one order less
       do i=pb,pe
!drho for lap(vor)
            call laplacian(vor(0,0,i),drho(0,0,i),mz,i)
!rhst for lap(phi)
            call laplacian(phi(0,0,i),rhst(0,0,i),mz,i)
       enddo
!-------------ACHTUNG---INCOMPRESSIBLE------------------------------!
...
!ACHTUNG---INCOMPRESSIBLE FLOW----------------------------------------!
!MODES 00
!
          if (myid.eq.0) then
             do j=0,my1
                 rf0u(j)=ire*mxwk(j,0,1)
                 rf0w(j)=ire*mzwk(j,0,1)
             enddo
          endif
!
!        
          call deryyr(rf0u,rf0u)
          call deryyr(rf0w,rf0w)
!rf0u,rf0w has the viscosity terms(second derivatives of u,w
!-------------------------------------------------------------------_!
...
!       6b.3) Calculate Nx,Nz
     ...
     .                         xbet(k)*nzwk(j,k,i)
            ...
            enddo
         enddo 
!ACTHUNG FOR INCOMPRESSIBLE ONLY
!----------------------00 MODES-------------------------------------!
!Save / compute terms needed for evolution of 00 modes
         if (myid.eq.0) then !only proc 0 does this
!debug flag
!           write(*,*) "M00x",(u00wk(j),j=0,my-1)
            do j=0,my-1
                rf0u(j)=rf0u(j)+real(nxwk(j,0,1))
!              !RHS(rhou00)
                rf0w(j)=rf0w(j)+real(nzwk(j,0,1))
!               !RHS(rhow00)
            enddo
...
! Calculate all tensor Sij
          ...
          do i=pb,pe
            do k=0,mz1
               do j=0,my-1
                  !S12
                  ten12(j,k,i)=0.5*(ten12(j,k,i)+
     .                              xalp(i-1)*mywk(j,k,i))
                  !S13
                  ten13(j,k,i)=0.5*(xbet(k)*mxwk(j,k,i)+
     .                              xalp(i-1)*mzwk(j,k,i))
                  !S23
                  ten23(j,k,i)=0.5*(ten23(j,k,i)+
     .                              xbet(k)*mywk(j,k,i))
               enddo
            enddo
          enddo
...
!ACHTUNG WE WILL NEED THESE LINE FOR THE PROPER LOMACODE
!----------------------00 MODES-------------------------------------!
!!Save / compute terms needed for evolution of 00 modes
!         if (myid.eq.0) then !only proc 0 does this
!!debug flag
...
! 
         do i=pb,pe
            do k=0,mz1
               k1 = icx(k)
               rK = bet2(k1)+alp2(i-1)
!WARNING: check rK sign!
               do j=0,my-1
!ACTHUNG --only incompressible-----------------------------------------!
                  mywk(j,k,i)= -rK*nywk(j,k,i) !aux1
                  nywk(j,k,i)= xalp(i-1)*nxwk(j,k,i)+
     .                         xbet(k)*nzwk(j,k,i) !aux2)
!----------------------------------------------------------------------!
...
! RHS(Omega_y)=ikzNx-ikxNx+ire*(ikzMx-ikxMz) 
         do i=pb,pe
            do k=0,mz1
               do j=0,my-1
!ACHTUNG----------!RHS(Omega_y)!INCOMPRESSIBLE ONLY
                  ten12(j,k,i)=xbet(k)*nxwk(j,k,i)-
     .                         xalp(i-1)*nzwk(j,k,i)+ire*drho(j,k,i)
!----------------- !RHS(phi) !INCOMPRESSIBLE ONLY------------------------
                  ten13(j,k,i)=mywk(j,k,i)-nywk(j,k,i)+ire*rhst(j,k,i)
!----------------------------------------------------------------------
!                  !RHS(Omega_y)
...
!debuggin RHS
        if (rkstep.eq.1.and.pb<2) then
               ...
               write(*,*) "my=0," , ten13(0,1,2)
               write(*,*) "my=257,", ten13(257,1,2)
               write(*,*) "RHS(vor),mode 1,1:" 
               write(*,*) "my=0,", ten12(0,1,2)
               write(*,*) "my=257,", ten12(257,1,2)
        endif
