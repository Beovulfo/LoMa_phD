...
!aaf disabling spwk because of bounds problems   
...
168      read(19,'(a)') text
         ...
         close(19)
         do iproc=1,numtot-1
!aaf send filenames too
            call MPI_SEND(filstt,70,MPI_CHAR,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(dat,20,MPI_REAL,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(idat,30,MPI_INTEGER,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
         enddo
      else
!aaf adding file names to receive from slaves
         call MPI_RECV(filstt,70,MPI_CHAR,0,
     &                 MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(dat,20,MPI_REAL,0,
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(idat,30,MPI_INTEGER,0,
     &                 MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
      endif
      ...
      bet  = dat(3)
      a0   = dat(4) 
      Deltat = 0.001D0     ! this data is not used 
      cfl  = dble(dat(6))
      nstep=idat(5)
      nimag=idat(6)
      nhist=idat(7)
