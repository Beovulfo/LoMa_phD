...
!!debug print to file
       if (myid.eq.0) then
        if (istep.eq.1.and.rkstep.eq.1) then
          open(unit=32,file="checklapT.txt",status="unknown")
        else
          open(unit=32,file="checklapT.txt",
     .                       status="old",position="append")
        endif

!debug  printing to file u(i,j) 
         write(32,*) 'istep = ', istep, 'rkstep = ', rkstep
!         write(32,*) 'rho v for i=0'
         write(32,*) 'lapT for i=0'
         write(32,*) 'Mode 00'
               do j=0,my-1
                 write(32,*) nywk(j,0,1)
               enddo
...
!debuggin: setting rhst to zero, now is just like having CONT. TEMP
       do i=pb,pe
          do k=0,mz1
             do j=0,my1
...
!!debug print to file
       if (myid.eq.0) then
        if (istep.eq.1.and.rkstep.eq.1) then
          open(unit=34,file="check2rhst.txt",status="unknown")
        else
          open(unit=34,file="check2rhst.txt",
     .                       status="old",position="append")
        endif
!debug  printing to file u(i,j) 
         write(34,*) 'istep = ', istep, 'rkstep = ', rkstep
         write(34,*) 'rhst for i=0'
         write(34,*) 'Mode 00'
               do j=0,my-1
                 write(34,222) rhst(j,0,1)
               enddo
         write(34,*) 'mode 01'
               do j=0,my-1
                 write(34,222) rhst(j,1,1)
               enddo
         close(34)
       endif
!.................................................................:!
...
!First set to zero chwk2, k=0,mgalz-1
       do i=pb,pe
           do k=0,mgalz-1
              do j=0,my-1
...
!*******************************************************************!
!
!ACHTUNG, not evolving PSI
!!------------(8b) Obtain psi(n+1)-----------------------------------!
!!Right now, for incompressible simulation we assume psi(n+1)=psi(n)
!
...
!       if (myid.eq.0) then
!        if (rkstep.eq.1) then
!          open(unit=40,file="check14.txt",status="unknown")
!!debug  printing to file u(i,j) 
!         write(40,*) 'v (up2),just before exit foup'
!         write(40,*) 'Mode 00'
!               do j=0,my-1
!                 write(40,*) up2(j,0,1)
!               enddo
...
!         !Calculate rhs(T)
         do i=0,mgalx-1
!!debuggin using simplified incompressible version of Energy Eq.
!            rhstwk(i)=-tmp1wk8(i)*ten12wk8(i)-tmp2wk8(i)*ten13wk8(i)-
!     .      tmp3wk8(i)*ten23wk8(i)+ !-u_i*dT/dxi
!     .      +1d0/peclet*ten22wk8(i) !1/pe*lap(T)
...
!         !Calculate the cross products rhou_i*u_j
         do i=0,mgalx-1
            ...
            ten23wk(i)=up3wk8(i)*tmp2wk8(i) !rhovw ready for ouput
         enddo
!
...
!!!debug print to file
!      if (myid.eq.0) then 
!       if (rkstep.eq.1.and.j.eq.1) then
!        open(unit=38,file="check13.txt",status="unknown")
!        write(38,*) 'v for 00 mode'
!       else
!        open(unit=38,file="check13.txt",status="old",position="append")
!       endif
!!debug  printing to file u(i,j) 
!           if (j.eq.1.or.mod(j-1,mgalz).eq.0) then
!              write(38,*) up2(0,jj)
!           endif
!        if (jj.eq.my) then
!           close(38)
