...
!!!===================================================!
!!!!!Set BC for phi, d(psi)/dy, d(Omegay)/dy
!         do i=pb,pe
!            do k=0,mz1
!!                    !PHI = 0 @ Boundaries
...
!!!
!!!!!                    !d(psi)/dy=0
!!!!!                    psi(my1-1,k,i)=psi(my1-2,k,i)
!!!!!                    psi(my1  ,k,i)=psi(my1-1,k,i)
!!!!!
!!!!!                    psi(1    ,k,i)=psi(2    ,k,i)
!!!!!                    psi(0    ,k,i)=psi(1    ,k,i)
!!!!
!!                    !dOmegay/dy=0
!                    vor(my1-1,k,i)=vor(my1-2,k,i)
...
!----------debugging----------------------------------------------!          
          if (myid.eq.0.and.istati.eq.1) then
            open(44,file='my00.txt', status='unknown')
            write(44,169) (y(j), real(mywk(j,0,1)),j=0,my1) 
            call flush(44)
          endif
!
...
!------------(8b) Obtain psi(n+1)-----------------------------------!
!Option 2
!1)prepare eta variable
          do i=pb,pe
             do k=0,mz1
               do j=0,my1
                  drho(j,k,i)=-drho(j,k,i)/dtbeta
               enddo
             enddo
          enddo 
!----------debugging----------------------------------------------!          
          if (myid.eq.0.and.istati.eq.1) then
            open(42,file='drho00.txt', status='unknown')
            write(42,169) (y(j), real(drho(j,0,1)),j=0,my1) 
            call flush(42)
          endif
!

!2) solve Lapvdv of Eta
         do i=pb,pe
            do k=0,mz1
               k1 = icx(k)
               rK = bet2(k1)+alp2(i-1)
               call Lapvdv2(drho(0,k,i),drho(0,k,i),ten23(0,k,i),rK)
            enddo
         enddo
!3)Solve psi(n+1)
          do i=pb,pe
...
!----------debugging----------------------------------------------!          
          if (myid.eq.0.and.istati.eq.1) then
            open(40,file='psi00.txt', status='unknown')
            write(40,169) (y(j), real(psi(j,0,1)),j=0,my1) 
            call flush(40)
          endif
169        format(d22.14,X,d22.14 )
!-----------------------------------------------------------------!

!!!!Clean PSI00 taking integral over Ly
!       if (myid.eq.0) then
!            intpsi00=cmplx(0.0,0.0)
!            do j=0,my1
!              intpsi00=intpsi00+psi(j,0,1)*hy(j+1)
!            enddo
!            intpsi00=intpsi00/(abs(y(my1)-y(0)))
!            !I=2Ly*H (equivalent rectangle)
!            !resting same height to every point
!            do j=0,my1
!              psi(j,0,1)=psi(j,0,1)-intpsi00
!            enddo
!      endif
!!
...
!      hyy=2d0/Re*(max(1d0/minval(Dy),max(hxalp,hzbet)))**2
      hyy=2d0/Re*(max(1d0/minval(Dy),max(hxalp,hzbet)))**2
...
! ----------------------------------------------------------------------!
         ...
         do i=0, mgalx-1
            up1wk(i)  = 1D0/up2wk8(i)-1D0/up1wk8(i)           !drho ready
