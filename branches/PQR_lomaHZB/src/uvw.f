...
! ----------------------------------------------------------------------!
     ...
     .    dwdy, scal2,dscal,dmfz,wk,nvecy,i)
      use wave
      use combustion
      use fis
     ...
     &          u,v,w,dudy,dvdy,dwdy,dscal,dmfz,mfzbis,scal2
        
      !------------------------Aux arrays    ----------------------------!
      integer k,j,k1,ind,ii,ip,nvecy
      complex*8 hgg (my),hvv (my),dyvor(my),o1(my),o3(my),o2(my),
     ...
     .          + xb*psi(j,k,ind) 
       ...
       do ind=0,nvecy-1
          ...
          !Calculating derivatives of rhoU,rhoV
             ...
             call deryr2(v(1,k,ind),o2,1)
             call deryr2(w(1,k,ind),o3,1)
             do j=1,my
                !rhov = my + d(psi)/dy
                dudy(j,k,ind) = o1(j)    !drhou/dy 
                dwdy(j,k,ind) = o3(j)    !drhow/dy 
                dvdy(j,k,ind) = o2(j) !   drhov/dy
                scal2(j,k,ind) = scal(j,k,ind) !T 
