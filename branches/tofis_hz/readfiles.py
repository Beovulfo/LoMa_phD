...
def readfieldij(filename,pert=0):
	""" 
	...
	#READING Differently depnding of plane typ
		...
		FIELD1=np.ndarray(shape=(mgalx,my),\
			 dtype=float, order='F')
		mean=np.ndarray(shape=(my),\
			 dtype=float, order='F')
		for j in range(my):
    			readrec = np.fromfile(f,rectx,1)
    			rectxdata = readrec['data']
			FIELD1[:,j] = rectxdata[0,:]
			mean[j] = np.mean(rectxdata[0,:])
			if pert==1:
				FIELD1[:,j]=FIELD1[:,j]-mean[j]
...
def worksta(filename,opt=1):
	...
	eta = (nu[pos_ml]**3/epR[pos_ml])**0.25 #eta=(nu^3/eps)^(1/4) 
	for j in range(0,len(pos_ml)-1):
		if eta[j]==0.0:
			dyeta[j]=0.0
		else:
			if j==0:
	...
	'Tm':Tm,'rum':rum,'rvm':rvm,'rwm':rwm,'rhom':rhom,'XI':XI,'nu':nu,'nacum':nacum}
