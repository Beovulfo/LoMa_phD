...
c--------------- initializes things 
     ...
     .             dwdy,dscal,dmfz,wk,plx,i)
         ...
         call swap(dvdy,vor,vor,i,plx,6)  ! H **
         call swap(mfzbis,vor,vor,i,plx,7)  ! Z
         call swap(dscal,vor,vor,i,plx,8)  ! dH/dy
         call swap(dmfz,vor,vor,i,plx,9)  ! dZ/dy **
     ...
     %           4*((9*nbuffou+11*nbuffis)/1048576),'MB'
      ...
      do j=1,my
         ...
         call getswapyz2xz(dvdy ,pr,pr,j,6) !scal
         call getswapyz2xz(mfzbis ,pr,pr,j,7)!Z
         call getswapyz2xz(dscal,pr,pr,j,8) !dH/dy
         call getswapyz2xz(dmfz,pr,pr,j,9) !dZ/dy
...
!         write(*,*)'chequeando',ind,j,jspecy(ind)
      ...
      ! escrphys subroutine
      call escrphys(u,v,w,dudy,dwdy,dvdy,mfzbis,dscal, dmfz,
