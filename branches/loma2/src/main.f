...
!--------------  allocates spectra and buffers
      ...
      nwkasize  = 12*nbuffsize + 12*my + 6*nbuffsize
     .            + nbuffsizephys
...
!-------+6 buffers-------------------!
      itnext  = itau23  + nbuffsize
      ichwk   = itnext  + nbuffsize
!-------+2 buffers-------------------!
...
167      read(19,'(a)') text
     ...
     &                 MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
      ...
      sigma = dat(4)
      !a0   = dat(4) 
      Deltat = 1e-4     ! this data is not used 
      cfl  = dat(5)
      nstep=idat(1)
      nimag=idat(2)
      nhist=idat(3)
