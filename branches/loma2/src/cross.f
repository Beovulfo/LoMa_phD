...
!-------------(6) Save Evolved Temperature ----------------------------!       
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
                  scal(j,k,i) = tnext(j,k,i)
                enddo
             enddo
          enddo
          !tnext now can be used
...
!-------------(7a) Obtain tauij ----------------------------!       
          ...
          do i=pb,pe
                call deryr2(mywk(0,0,i),tnext(0,0,i),mz)
          enddo
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
                  !tau22= 2*d(T^sigma*v/Re)/dy-tau22B
                  tau22(j,k,i) =2*tnext(j,k,i)-tau22(j,k,i)
                enddo
             enddo
          enddo
...
!-------------(7b) Obtain tauij (2/2)--------------------------!       
     ...
     .               +xbet(k)*mxwk(j,k,i)-tau13(j,k,i)
          ...
          do i=pb,pe
                call deryr2(mxwk(0,0,i),tnext(0,0,i),mz)
          enddo
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
     ...
     .                +xalp(i-1)*mywk(j,k,i)-tau12(j,k,i)
          ...
          do i=pb,pe
                call deryr2(mzwk(0,0,i),tnext(0,0,i),mz)
          enddo
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
...
!======================================================!
      ...
      if (istati.eq.1.and.rkstep.eq.1) then
           ...
           do i=pb,pe
             do k=0,mz1
                do j=0,my1
                   mxwk(j,k,i)=mxwk(j,k,i)/ire
                   mywk(j,k,i)=mywk(j,k,i)/ire
                   mzwk(j,k,i)=mzwk(j,k,i)/ire
                enddo
            enddo
           enddo
!=================================================================
...
!NOW WE DONT HAVE VORTICITES OR VELOCITIES!!
!!*******************************************************************!
          !Accumulating stats while istati==1
          nacum = nacum +1
          nacumsp = nacumsp +1
...
!!-----STATS NEED TO BE REVISITED!!

!-------------(7) Obtain aux1, aux2, aux3-------------------------!       
!........AUX1=d/dy (d/dz(-rhouv+tau12)+d/dx(rhovw-tau32))
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
                  mxwk(j,k,i) =xbet(k)*(-ten12(j,k,i)+tau12(j,k,i))+
...
!                                    (rhovv-tau22)
     ...
     .         -  tau11(j,k,i)-nywk(j,k,i)+tau22(j,k,i))
     .         +  (-bet2(k1))*(nzwk(j,k,i)-tau33(j,k,i)
     .                       -nywk(j,k,i)+tau22(j,k,i))
...
!==============================================================
!
!-------(9) Obtaining  RHS(Omegay), RHS(phi)---------------
!        !RHS(Omegay)= AUX1+
...
!-------------------------------------------------------
         do i=pb,pe
            do k=0,mz1
               ...
               rK = bet2(k1)+alp2(i-1)
               do j=0,my-1
                  mywk(j,k,i)=mywk(j,k,i)+mzwk(j,k,i)
...
c     ---------------------- Variables ----------------------
     ...
     .          ten13,ten23,tnext,chwk

      complex(4), dimension(0:my1,0:mgalz-1,pb:pe):: 
     .      tau11,tau22,tau33,tau12,tau13,tau23,drho
...
!temperature
      call chpl2ln(ten22,ten22,chwk,myid)!lap(T)
      call chpl2ln(ten12,ten12,chwk,myid)!dT/dx
      call chpl2ln(ten13,ten13,chwk,myid)!dT/dy
      call chpl2ln(ten23,ten23,chwk,myid)!dT/dz
      call chpl2ln(drho,drho,chwk,myid)!T
...
c     --------------------------- Variables -------------------      
     ...
     .        tau23(0:mx-1,lb:le),
     .        tnext (0:mx-1,lb:le),drho(0:mx-1,lb:le)
      real(8),dimension(0:mgalx-1,lb:le)::rhst
      real(8) pi
      !aaf adding Peclet
      real(8) :: peclet,T
...
!         !Calculate u,v,w
         do i=0,mgalx-1
            ...
            up1wk(i)  = ire*tmp1wk8(i)*rhstwk8(i)**sigma  
            !T^sigma*u for output

            tmp2wk8(i)= up2wk8(i)*rhstwk8(i) !v
            up2wk(i)  = ire*tmp2wk8(i)*rhstwk8(i)**sigma  
            !T^sigma*v for output

            tmp3wk8(i)= up3wk8(i)*rhstwk8(i) !w for wk
            up3wk(i)  = ire*tmp3wk8(i)*rhstwk8(i)**sigma  
            !T^sigma*w for output
         enddo
