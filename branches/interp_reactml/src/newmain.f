...
!      ----------------------- Variables -------------------------------
     ...
     .        irf0v,iv00wk,izeta,idhache,idzeta,ilapz,
     .        ichwk,ichwk2,irhst,idrho,nstr,nspsize,iflag
      integer ivor,iphi,ipsi,iscal,imfz
...
!--------------- initializes commons and things 
       ...
       call initcr(myid)
       !now some calculations for combustion
       if (myid.eq.0) then
          S= 1d0/Zs-1d0
          Zsbar=1d0/(S/Lf+1d0)
          iLm = (S/Lf+1d0)/(S+1d0)
          Hf = T0f-1d0-gam*(S+1d0)/S
          write(*,*) "S=",S,"Zsbar=",Zsbar,"Hf=",Hf
      !Calculate integration constant for Tsmooth
          cteintT = gam*(S+1d0)/S-gam/((1d0-Zs)*Zs)*smootht(1d0)
       endif
      
!--------------  allocates spectra and buffers
      nspsize   = (nspec+1)*(nz1+1)*12*(pe-pb+1)
      nbuffsize = max((pe-pb+1)*2*my*mgalz,(le-lb+1)*mx)
      nwkasize  = 8*nbuffsize + 12*my + 10*nbuffsize
...
!      nwkasize  = 7*nbuffsize + 8*my + 6*nbuffsize
      ...
      izeta   = iten23  + nbuffsize
      idhache = izeta   + nbuffsize
      idzeta  = idhache  + nbuffsize
      ichwk   = idzeta  + nbuffsize
...
!      ---------------  read from file and distribute
     ...
     .   'bete=',bete,'mxe=',mxe,'mye=',mye,'mze =', mze
         ...
         !ntotr=4*mye*mze
         ntotr=10*mye*mze !vor phi psi scal x2
!     ------------- 00 modes ------------------
