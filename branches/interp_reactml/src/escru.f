...
!    added mfz by aaf 29/07/15
!                                                                  !
!                                                                  !
!******************************************************************!
      !subroutine escru(vor,phi,psi,scal,mfz,u00,w00,v00,myid)
      subroutine escru(vor,phi,psi,scal,mfz,u00,w00,v00,myid)
      use tem
      use fis
      use ficheros
...
c  ------------------------- Variables ----------------------------
      ...
      real(realprec) :: vor(0:(2*mye-1),0:mz-1,pb:pe),
     .        phi(0:(2*mye-1),0:mz-1,pb:pe),
     .        psi(0:(2*mye-1),0:mz-1,pb:pe),
     .        scal(0:(2*mye-1),0:mz-1,pb:pe),
     .        mfz(0:(2*mye-1),0:mz-1,pb:pe)

      real(realprec) ::  u00(mye),w00(mye),v00(mye)
      real(4) ::  wk00(3*mye)
...
c    ----------------------- Statistics ----------------------         
      ...
      if(myid.eq.0) then 
         call MPI_SEND(u00,mye,MPI_DOUBLE_PRECISION,
     .                  numerop,1,MPI_COMM_WORLD,ierr)
         call MPI_SEND(w00,mye,MPI_DOUBLE_PRECISION,
     .                  numerop,2,MPI_COMM_WORLD,ierr)
         call MPI_SEND(v00,mye,MPI_DOUBLE_PRECISION,
     .                  numerop,3,MPI_COMM_WORLD,ierr)
      elseif(myid.eq.numerop) then 
         call MPI_RECV(u00,mye,MPI_DOUBLE_PRECISION,
     .                  0,1,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(w00,mye,MPI_DOUBLE_PRECISION,
     .                  0,2,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(v00,mye,MPI_DOUBLE_PRECISION,
     .                  0,3,MPI_COMM_WORLD,istat,ierr)
      ...
      if (myid.lt.numerop) then    ! calc proc
         do i=pb,pe
           call MPI_SEND(vor(0,0,i),leng,MPI_DOUBLE_PRECISION,plsav(i),
     .                    plsav(i),MPI_COMM_WORLD,ierr) 
                 
           call MPI_SEND(phi(0,0,i),leng,MPI_DOUBLE_PRECISION,plsav(i),
     .                    plsav(i),MPI_COMM_WORLD,ierr)

           call MPI_SEND(psi(0,0,i),leng,MPI_DOUBLE_PRECISION,plsav(i),
     .                    plsav(i),MPI_COMM_WORLD,ierr)

           call MPI_SEND(scal(0,0,i),leng,MPI_DOUBLE_PRECISION,plsav(i),
     .                   plsav(i),MPI_COMM_WORLD,ierr)

           call MPI_SEND(mfz(0,0,i),leng,MPI_DOUBLE_PRECISION,plsav(i),
     .                    plsav(i),MPI_COMM_WORLD,ierr)
         enddo
         return
      else
         do i = pb,pe
          call MPI_RECV(vor(0,0,i),leng,MPI_DOUBLE_PRECISION,plcalc(i),
     .                 MPI_any_tag,MPI_COMM_WORLD,istat,ierr)
              
          call MPI_RECV(phi(0,0,i),leng,MPI_DOUBLE_PRECISION,plcalc(i),
     .                 MPI_any_tag,MPI_COMM_WORLD,istat,ierr)

          call MPI_RECV(psi(0,0,i),leng,MPI_DOUBLE_PRECISION,plcalc(i),
     .                 MPI_any_tag,MPI_COMM_WORLD,istat,ierr)

          call MPI_RECV(scal(0,0,i),leng,MPI_DOUBLE_PRECISION,plcalc(i),
     .                 MPI_any_tag,MPI_COMM_WORLD,istat,ierr)

           call MPI_RECV(mfz(0,0,i),leng,MPI_DOUBLE_PRECISION,plcalc(i),
     .                 MPI_any_tag,MPI_COMM_WORLD,istat,ierr)
         enddo
      endif
     ...
     .         form='unformatted')
         ...
         write(iout) time,Ree,alpe,bete,a0e,mx,mye,mz,
     .               (ye(j),ye(j),j=1,mye),
     .               (wk00(j),j=1,3*mye)
...
!         !! master write its fields
         ...
         do i=pb,pe
            write(iout) ((real(vor(j,k,i),4),real(phi(j,k,i),4),
     .                   real(psi(j,k,i),4),real(scal(j,k,i),4),
     .                   real(mfz(j,k,i),4),j=0,2*mye-1),k=0,mz1)
         enddo
     ...
     .              iproc,iproc,MPI_COMM_WORLD,istat,ierr)
            ...
            call MPI_RECV(vor(0,0,1),leng,MPI_DOUBLE_PRECISION,
     .            iproc,iproc,MPI_COMM_WORLD,istat,ierr)            

            call MPI_RECV(phi(0,0,1),leng,MPI_DOUBLE_PRECISION,
     .            iproc,iproc,MPI_COMM_WORLD,istat,ierr)            

            call MPI_RECV(psi(0,0,1),leng,MPI_DOUBLE_PRECISION,
     .            iproc,iproc,MPI_COMM_WORLD,istat,ierr)            

            call MPI_RECV(scal(0,0,1),leng,MPI_DOUBLE_PRECISION,
     .            iproc,iproc,MPI_COMM_WORLD,istat,ierr)            

            call MPI_RECV(mfz(0,0,1),leng,MPI_DOUBLE_PRECISION,
     .            iproc,iproc,MPI_COMM_WORLD,istat,ierr)            
     ...
     .                 numerop,myid,MPI_COMM_WORLD,ierr)           
         ...
         call MPI_SEND(vor(0,0,pb),leng,MPI_DOUBLE_PRECISION,
     .                 numerop,myid,MPI_COMM_WORLD,ierr) 
     
         call MPI_SEND(phi(0,0,pb),leng,MPI_DOUBLE_PRECISION,
     .                 numerop,myid,MPI_COMM_WORLD,ierr)

         call MPI_SEND(psi(0,0,pb),leng,MPI_DOUBLE_PRECISION,
     .                 numerop,myid,MPI_COMM_WORLD,ierr)

         call MPI_SEND(scal(0,0,pb),leng,MPI_DOUBLE_PRECISION,
     .                 numerop,myid,MPI_COMM_WORLD,ierr)

         call MPI_SEND(mfz(0,0,pb),leng,MPI_DOUBLE_PRECISION,
     .                 numerop,myid,MPI_COMM_WORLD,ierr)
...
!-------------Variables----------------------------!
      ...
      real*4 fac
      character*100 fnamesta,fnamespe
      character*3 ext1
      real*4 Ree,alpe,bete,a0e
...
!spectra
     ...
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(thep,wkstats(1,19),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(theup,wkstats(1,20),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(Hp,wkstats(1,21),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(Zp,wkstats(1,22),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(mum,wkstats(1,23),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(num,wkstats(1,24),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
         call MPI_ALLREDUCE(chi,wkstats(1,25),my,MPI_DOUBLE_PRECISION,
     .                      MPI_SUM,MPI_COMM_CALC,ierr)
     ...
     .              rum(j),rvm(j),rwm(j),
     .              w1m(j),w2m(j),w3m(j),
     .              them(j), Tm(j),!added for compressibility
     .              Hm(j), Zm(j), !added for combustion
     ...
     .              wkstats(j,19)*fac,wkstats(j,20)*fac, !compres
     .              wkstats(j,21),wkstats(j,22), !comb
     .              wkstats(j,23)*fac,wkstats(j,24)*fac, !mum,num
     .              wkstats(j,25)*fac, !chi
     .                                             j=1,my)
         close(isn)
      endif
c   ----------------------- Spectra --------------------
