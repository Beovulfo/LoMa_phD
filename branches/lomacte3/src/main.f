...
!      ----------------------- Variables -------------------------------
     ...
     .        ichwk,ichwk2,irhst,idrho,nstr,nspsize,iflag
      integer ivor,iphi,ipsi,iscal,ideb1,ideb2
...
!--------------  allocates spectra and buffers
      ...
      nwkasize  = 8*nbuffsize + 12*my + 6*nbuffsize+
     .           nbuffsizephys
      !nwkasize  = 8*nbuffsize + 12*my + 6*nbuffsize
...
!      nwkasize  = 7*nbuffsize + 8*my + 6*nbuffsize
      ...
      !DEBUG
      !ideb1   = ichwk  + nbuffsize
      !ideb2   = ideb1  + nbuffsize
     ...
     .               sp,myid)
      ...
      if (myid.eq.0) then
        write(*,*) "Master at Last BARRIER before end"
      endif
      call MPI_BARRIER(MPI_COMM_WORLD,ierr)     
      if (myid.eq.0) then
        write(*,*) "Master after last BARRIER before end"
      endif
...
167      read(19,'(a)') text
     ...
     &                 MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
      ...
      nstart = idat(6)
      ntimes = idat(7)
      peclet = Re*0.7 !assuming Prandtl = 0.7
      sigma  = 0.0 !T^sigma mu0 is the viscosity
