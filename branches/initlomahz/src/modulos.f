...
c fis AAF
      ...
      !real(8), public ::  smooth,Temper,dZbdZ,dZbdZ2,dTdZ
!     FUNCTIONS 
      contains
        function smooth(Z)
        real(8),intent(in):: Z
        real(8)::smooth
        smooth =1d0/(2d0*betha)*(
     .  betha*Z-log(cosh(betha*(Zs-Z)))+log(cosh(betha*Zs)))
        ...
        real(8)::Temper
        Temper =cteintT+1d0+H+(gam/((1d0-Zs)*Zs))*smooth(Z)
        end function
        function dZbdZ(Z)
        real(8),intent(in):: Z
        real(8)::dZbdZ
     ...
     .              betha*0.5d0/(cosh(betha*(Z-Zs)))**2
      ...
      !need to include to read values from hre.dat
