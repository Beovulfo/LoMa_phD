...
!------------------
      ...
      do j=1,my
          ...
          ufou(j,1,1) = rho00(j)*u0*tanh(-y(j)/(2d0*dm0)) !rhou00
          vfou(j,1,1) = 0.0
          wfou(j,1,1) = 0.0
      enddo

       !show Density ratio
       write(*,*) "Density ratio s12 = ",real(scal(1,1,1)/scal(my,1,1))
       write(*,*) "Density ratio s21 = ",real(scal(my,1,1)/scal(1,1,1))
...
! SCAL
      do i = 1, nplanes
         ...
         enddo
      enddo
!mode00 only one different to zero
      if (s.eq.1d0) then
      ...
      !s can be used to multiply scalar max
         ...
         do j=1,my
            scal(j,1,1) = cmplx(real(rho00(j)),0.0)
         enddo
      endif
