...
!       -changed to Explicit code, and added multiple                   !
!        passive scalar equations (02/2014)                             !       
!                                                                       ! 
...
!      ----------------------- Variables -------------------------------
     ...
     .        iuwk,ichwk,nstr,nspsize,iflag,iscal1,iscal2,iscal3
     
      integer itags,newtag,imess,i,comproc(0:numerop-1),
     .                      savproc(numerop:numtot-1),iscal      
      
      real*8  u00(my),w00(my),val 
      real*4, allocatable:: vor(:),phi(:),wk(:),sp(:),scal(:)
...
!--------------  allocates spectra and buffers
      ...
      nwkasize  = 7*nbuffsize + 8*my +3*nscalars*nbuffsize
      !adding 3 scalar wk fields for each scalar
      allocate(sp(nspsize))
      ...
      if (myid.lt.numerop) then     ! Comp. Proc.
         ...
         allocate(scal(nscalars*2*my*mz*(pe-pb+1)))
         allocate(wk(nwkasize))

      else                          ! Save proc
!          nwkasize = 4*mmp*my*mz+1000
          nwkasize = (4+2*nscalars)*mmp*my*mz+1000
! WTF is this for SAVE Proc? later use nwkasize/2...
          allocate(wk(nwkasize))
...
!getscal has to be reviewed
! CAUTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        do iscal=1,nscalars
          call getscal(scal(2*my*mz*mmp*(iscal-1)+1),wk,iscal,myid)
        enddo
...
!added by aaf
      iscal1   = ichwk  + nbuffsize
      iscal2   = iscal1 + nscalars*nbuffsize
      iscal3   = iscal2 + nscalars*nbuffsize
...
! ---------------------- variables -------------------------------------
      ...
      parameter(ndat=5+nscalars,nidat=7)
...
!        idat(1:3)=[nstep,nimag,nhist]


967      read(19,'(a)') text
         if(text(1:2).eq.'CC') goto 967
         read(text,*) dat(5), idat(4)
!        dat(5)=CFL, idat(4)=ncfl

968      read(19,'(a)') text
         if(text(1:2).eq.'CC') goto 968
         read(text,*) (idat(j), j=5,7)
!        idat(5 )=first output file
!        idat(6)=stats(0=no stats)
!        idat(7)=#steps between stats 
!read coefficients for scalar equations
969      read(19,'(a)') text
         if(text(1:2).eq.'CC') goto 969
         read(text,*) (dat(j+5),j=1,nscalars)
!        dat(6)=scadiff
...
c ---------------  modes values for FOURIER
     ...
     .               (y(j),fmap(j),j=1,my)
         rewind(iinp)
         close(iinp)
! read filinp file just to take y vector , my and fmap (fmap will be
! changed later on and computed from y in 'derivadas')
         do iproc=1,numtot-1
...
!  taking care of the plane "i".
         ...
         j=0
         do i=1,nplanes
!aaf check why here we are using aux2 instead of pend
...
!***********************************************************************!
     ...
     .       spwk(0:nz1,1:  nspec+1,8,pb:pe)
      ...
      integer myid,istep,irun,rkstep,i,k,j,k1,ierr,nanerror,kk,iscal
...
!     Call assign
      ...
      do iscal=1,nscalars
          call getscal(scal(0,0,pb,iscal),chwk,iscal,myid)
      enddo
...
c         ------------  other modes,  master node -----
         ...
         do i=1,(pe-pb+1)
            read(iinp) (wk1(j),j=1,ntotr)
            call assignscal(wk1,scal(1,1,i),my,mz)
         enddo
...
!         ------------  other modes,  distribute to slaves --
         ...
         do iproc=1,numerop-1
           mmy2 = min(pend(iproc),mx/2)-pbeg(iproc)+1
!           write (*,*) mmy2,pend(iproc),mxe/2
           if (mmy2.gt.0) then
              do i=1,mmy2
                 read(iinp) (wk1(j),j=1,ntotr)
