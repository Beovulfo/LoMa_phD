...
!      ----------------------- Variables -------------------------------
     ...
     .        ichwk,ichwk2,irhst,idrho,nstr,nspsize,iflag
      integer ivor,iphi,ipsi,iscal,imfz,ilapz,irhsz
      integer itau11,itau22,itau33,itau12,itau13,itau23
      integer idhache,idzeta
...
!--------------  allocates spectra and buffers
      ...
      nbuffsizephys =2*(le-lb+1)*mgalx
      nwkasize  = 20*nbuffsize + 12*my +
     .            + 2*nbuffsizephys
...
!      nwkasize  = 7*nbuffsize + 8*my + 6*nbuffsize
      ...
      if (myid.lt.numerop) then
         ...
         enddo
          ...
          call getfil(vor,phi,psi,scal,mfz,u00,v00,w00,wk,myid)
          !Sanity check to avoid noise at beginning
          if (gam.eq.0.0) then
            if (myid.eq.0) then
               write(*,*) "Setting psi to zero because gam=0."
            endif
          !Make PSI = 0
            do i=1,2*my*mz*mmp
               psi(i) = 0. 
...
!-------+3 buffers-------------------!
      itau11  = iten23  + nbuffsize
      itau22  = itau11  + nbuffsize
      itau33  = itau22  + nbuffsize
      itau12  = itau33  + nbuffsize
      itau13  = itau12  + nbuffsize
      itau23  = itau13  + nbuffsize
!-------+6 buffers-------------------!
      idhache = itau23  + nbuffsize
...
!      ---------------  read from file and distribute
     ...
     .   'bete=',bete,'mxe=',mxe,'mye=',mye,'mze =', mze
         ...
         x0 = 1E-8
         y0 = 1E-8
         Lx = 2.0*(4.0*atan(1d0))/alpe
         do i=1,mgalx
            x(i) = -Lx/2+(Lx/(mgalx-1))*(i-1)
         enddo

         !ntotr=4*mye*mze
         ntotr=5*2*mye*mze !vor phi psi scal mfz x2
!     ------------- 00 modes ------------------
...
!     ------------ 00 modes only for master 
     ...
     &                iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(alpe,1,MPI_INTEGER,iproc,
     &                iproc,MPI_COMM_WORLD,ierr)
         enddo
...
!         ------------  other modes,  distribute to slaves --
     ...
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(alpe,1,MPI_INTEGER,master,
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
...
c ---------------  modes values for FOURIER
     ...
     .               (y(j),fmap(j),j=1,my)
         rewind(iinp)
         close(iinp)
         time0 = 0.001 !initial time before vortex
     ...
     &                 iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(time0,1,MPI_REAL4,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
     ...
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(time0,1,MPI_REAL4,0,
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
...
c --------------  write header for output -------------
     ...
     .     'Delt =',Deltat,'  CFL =',CFL
         write(*,*)
         write(*,'(a10,e12.4,a8,f5.2,a8,f5.2)')
     .     'S =',S,'Lf=',Lf, 'betha =',betha
         write(*,*)
         write(*,'(a,a)')
     .     'reading from:  ',filinp
         write(*,*)
