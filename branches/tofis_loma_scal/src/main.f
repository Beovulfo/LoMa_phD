...
!     AAF     10/07/14  Modified for Compressible Flow(LOMA)            !
!     AAF     10/08/15  Modified for LOMACTE_SCAL (s=1.0)               !
!                                                                       !
!***********************************************************************!
...
c--------------- initializes commons and things 
     ...
     .               wk,plx,i)
         ...
         call swap(dvdy,vor,vor,i,plx,6)  ! T
      ...
      deallocate(vor,phi,psi,scal,u,v,w,dudy,dvdy,dwdy,dscal) !,wkb)
      ...
      write(*,*)'Allocated memory:',
     %           4*((9*nbuffou+7*nbuffis)/1048576),'MB'
...
!         write(*,*)'chequeando',ind,j,jspecy(ind)
      ...
      ! escrphys subroutine
      call escrphys(u,v,w,dudy,dwdy,dvdy,dscal,wk,
