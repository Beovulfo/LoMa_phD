...
!  Solve laplacian to find my and dmy/dy
         do i=pb,pe
            do k=0,mz1
               ...
               rK = bet2(k1)+alp2(i-1)
               !BCs
               !dmy/dy = 0 and my=0 as Boundary conditions
               call Lapvdvhom(phi(0,k,i),mywk(0,k,i),nzwk(0,k,i),rK)
               !call solvepsi(phi(0,k,i),mywk(0,k,i),nzwk(0,k,i),rK)
               !call Lapvdvhom(phi(0,k,i),mywk(0,k,i),nzwk(0,k,i),rK)
            enddo
         enddo
...
!---------- Only master computes 00  modes
        if (myid.eq.0) then
           do j=0,my1  
              ...
              !debug
              !mywk(j,0,1) = mywk(j,0,1)+v00wk(j) !rhov00 
              mzwk(j,0,1) = w00wk(j)
           enddo
...
!        enddo 
         ...
         enddo
         !!debug
         !!dP0/dt = 1/V*1/Pe*int(lapT)dV
         !if (myid.eq.0) then
         !   trapz=0.0
         !   do j=0,my1
         !      trapz =trapz+hy(j+1)*nywk(j,0,1) 
         !   enddo
         !   if (trapz.ne.0.0) then
         !       write(*,*) "dP0/dt is not zero!:"
         !       write(*,*) "int(lapT00)=",trapz
         !   endif
         !endif
...
! ten23= rhovw
! rhst = RHS(T(i+1)
!!! lappsi = lappsi*(Tn/T(n+1))^2 
! lappsi = lappsi at next step 
! drho = EMPTY!!DT/T(n+1)^2
!!!! drho = Drho
! chwk2 = T(i+1)
!---------------------------------------------------------------------!
...
!------------(8) Obtain lap psi(n+1)--------------------------------!
! update lappsi n+1
!         do i=pb,pe
!            do k=0,mz1
!               do j=0,my1
!                 lappsi(j,k,i) = drho(j,k,i)/dtbeta -
!     .           alpha(rkstep)/beta(rkstep)*lappsi(j,k,i)
!               enddo
!            enddo
!         enddo
...
!        1.1) Integrate drho00
              !mode 00 of lappsi is mean(drho/dy)
              do j=0,my1
                 drho00(j) = real(lappsi(j,0,1))
              enddo
              call inty8(drho00,rf0v)!integrate drhov
              !call cumtrapzK(drho00,rf0v,0.0)!integrate drhov
              trapz = rf0v(my1)
              !trapz = -rf0v(my1)
              !trapz = -rf0v(my1)
              !integrate drhovdy
              do j=0,my1
                 v00wk(j) = trapz*(1d0/(1d0+(rhotop/rhobot)**0.5))+
     .           rf0v(j)
              enddo
         endif
!----------------------------------------------------------------------
...
!   2.2) solve Lapvdv of PSI
         do i=pb,pe
            do k=0,mz1
               k1 = icx(k)
               rK = bet2(k1)+alp2(i-1)
               call solvepsi(lappsi(0,k,i),psi(0,k,i),ten23(0,k,i),rK)
...
324              format(17(d22.14))
                 write(29,324)  time,dm,enerdis, energy,
     .           v00wk(0)-v00wk(my1),massu,massw,
     .           MPI_WTIME()+iter_time, commtimer-ctm2
                 call flush(29) !for XLF
                 !call flush(29)

325              format(i5,13(d14.6))
                 write(*,325) istep,time,Deltat,dm,v00wk(0)-v00wk(my1),
     .                   massu,massw,
...
!       ten13   =      dT/dy          rhouw                        !
!       ten23   =      dT/dz          rhovw                        !
!       lappsi  =      lappsi        lappsi*(T(i)/T(i+1))^2   !
!                                                                  !
!******************************************************************!
...
c     ---------------------- Variables ----------------------
     ...
     .          ten13,ten23,drho,lappsi,tnext,chwk
...
!      !TEMPERATURE
      ...
      call localyz2zy(lappsi,lappsi,chwk) !lappsi    
...
! inverse transform (fou --> fis @z),does the compacting aswell
      do i = pb,pe
         ...
         call fourz(lappsi(0,0,i),1) ! lappsi
      enddo
...
!temperature
      ...
      call chpl2ln(lappsi,lappsi,chwk,myid)
...
c  ---------- back to the yz planes an fft 
      ...
      call chln2pl(tnext,tnext,chwk,myid) !T(i+1)
      call chln2pl(lappsi,lappsi,chwk,myid) !lappsi
!  convert to fourier Z now.
      do i=pb,pe
         ...
         call fourz(tnext(0,0,i),-1)   !T(i+1)
         call fourz(lappsi(0,0,i),-1)   !lappsi
      enddo
!transpose back
      ...
      call localzy2yz(lappsi,lappsi,chwk)
!------------------------------------------------------!
...
c     --------------------------- Variables -------------------      
     ...
     .        ten23(0:mx-1,lb:le),lappsi(0:mx-1,lb:le),
...
! upiwk8 means real 8 format
         ...
         call fourx(lappsiwk,lappsiwk8,1) ! lappsi
! ----------------------------------------------------------------------!
!                                                                       !
...
!            1/pe*T**(sigma+1)*lap(T)
         enddo         
!         !Build div(rui) with Nicouds constraint
         do i=0,mgalx-1
            lappsiwk(i) = 1.0/rhstwk8(i)*!rho*div(u)
     .      (1.0d0/peclet)*ten22wk8(i)!
     .       -(1.0d0/rhstwk8(i)**2)*
     .      (tmp1wk8(i)*ten12wk8(i)+tmp2wk8(i)*ten13wk8(i)+
     .      tmp3wk8(i)*ten23wk8(i)) !-rho^2*u_i*dT/dxi
         enddo

         !Evolve T (second part)
         do i=0,mgalx-1
             tnextwk8(i) = tnextwk8(i) + dtgamma*rhst(i,j)!tnext use
             tnextwk(i) = tnextwk8(i) !tnext for output
...
!         !Calculate the cross products rhou_i*u_j
         ...
         !save drhowk
         !do i=0,mgalx-1
         !   drhowk(i) = drhowk8(i)/(tnextwk8(i)**2)!
         !   !DT/T(i+1)^2
         !enddo

         !build lappsi*(T(n)/T(n+1))^2
         !do i=0,mgalx-1
         !   lappsiwk(i) = lappsiwk8(i)*(rhstwk8(i)/tnextwk8(i))**2
         !   !lappsi*(T(n)/T(n+1))^2
         !enddo
...
c--------------------- back to F-P-P  
         ...
         call fourx(lappsiwk,up1wk8,-1)  !lappsi(T(i)/T(i+1))^2
