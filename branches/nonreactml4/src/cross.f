...
c ------------------- Variables ----------------------------------------
     ...
     .          ten23(0:my-1,0:mgalz-1,pb:pe)
      ...
      complex(realprec) :: Tphys(0:my-1,0:mgalz-1,pb:pe)
      complex(realprec) :: dhache(0:my-1,0:mgalz-1,pb:pe)
      real(doubleprec) ::  dzeta(0:mgalx-1,lb:le)

      real(doubleprec) :: u00(0:my-1),w00(0:my-1),rf0u(0:my-1),
     .       rf0w(0:my-1),u00wk(0:my-1),w00wk(0:my-1),
     .       v00(0:my-1),v00wk(0:my-1),rf0v(0:my-1)
     
      real(doubleprec)  :: trapz 
      real(doubleprec)  :: drho00(0:my1),drhoxz(0:my1) 
      
      integer myid,istep,irun,rkstep,i,k,j,k1,ierr,kk,
     .        rksteptest
!spectra
      real(realprec)::  sp(0:nz1,1:  nspec+1,12,pb:pe),
     .       spwk(0:nz1,1:  nspec+1,12,pb:pe)

      
      
      real(doubleprec) :: iter_time,write_time
...
!Setting BC'S
         ...
         !       !These BCs are not well imposed for CFD scheme
                ...
                !potential flow, this should me zero
                !nxwk(0  ,k,i) = cmplx(0.0,0.0) 
                !nxwk(my1,k,i) = cmplx(0.0,0.0) 
...
!  Solve laplacian to find my and dmy/dy
         do i=pb,pe
            do k=0,mz1
               k1 = icx(k)
               rK = bet2(k1)+alp2(i-1)
               !potential flow - exponential decay
...
!                   !rhov=my+d(psi)/dy
!                   mywk(j,k,i)=mywk(j,k,i) !dpsi/dy is zero now    
...
!!---------!Use buffers for velocity------------------------------------!
         do i=pb,pe
            do k=0,mz1
               do j=0,my1
                  rhst(j,k,i)  =mxwk(j,k,i) !u
                  Tphys(j,k,i) =mywk(j,k,i) !v
                  dhache(j,k,i)=mzwk(j,k,i) !w
               enddo
            enddo
        enddo
...
!Here we compute the new Dt                                          !
        call foup2non(mxwk,mywk,mzwk,nxwk,nywk,nzwk,
     .          rhst,ten12,ten13,ten23,
     .          rhsz,drho,lapz,dzeta,
     .          chwk,sp,myid,rkstep)
!-----------------
...
!!!---------!Use buffers for velocity------------------------------------!
         do i=pb,pe
            do k=0,mz1
               do j=0,my1
                  mxwk(j,k,i) = rhst(j,k,i) !u
                  mywk(j,k,i)= Tphys(j,k,i) !v
                  mzwk(j,k,i)= dhache(j,k,i) !w
               enddo
            enddo
        enddo
!------------------------------------------------------!
...
!! dm will keep the momentum thickness
         ...
         if (myid.eq.0.and.rkstep.eq.1) then
            if (ihist.eq.1) then
              dm=0d0
              do j=1,my-2
                  dm=dm+trp(j+1)*((real(mxwk(j+1,0,1)))**2.0+
     .                          (real(mxwk(j-1,0,1)))**2.0)
              enddo
              dm=dm+trp(1)*((real(mxwk(1,0,1)))**2.0+
     .        (real(mxwk(0,0,1)))**2.0)+
     .        trp(my)*((real(mxwk(my1,0,1)))**2.0+
     .                  (real(mxwk(my-2,0,1)))**2.0) 

              dm =1d0/4.0d0*(y(my)-y(1))-
     .           dm/(real(mxwk(my1,0,1))-real(mxwk(0,0,1)))**2.0
             endif
          endif
...
! rhsz = Z(i+1)
        ...
        if (myid.eq.0) then
           do j= 0,my1
              drho(j,0,1) = 1d0 !Mode 00 of constant temperature
           enddo
        endif
!!! lapz = T(i)
!!! drho = Drho
...
!*******************************************************************!
          do i=pb,pe
             do k=0,mz1
                do j=0,my1
                    !TauIJ = 2.0*Sij !ignoring isotropic components that
                    !would vanish
                    mxwk(j,k,i) = 2.0D0*mxwk(j,k,i)
                    mywk(j,k,i) = 2.0D0*mywk(j,k,i)
                    mzwk(j,k,i) = 2.0D0*mzwk(j,k,i)
                    ten12(j,k,i) = 2.0D0*ten12(j,k,i)
                    ten13(j,k,i) = 2.0D0*ten13(j,k,i)
                    ten23(j,k,i) = 2.0D0*ten23(j,k,i)
                enddo
             enddo
          enddo
...
!         First we can calculate My
       do i=pb,pe
          call deryyr2(mywk(0,0,i),mywk(0,0,i),mz) !now mywk=d2(v)/d2y
       enddo
       do i=pb,pe
          do k=0,mz1
             do j=0,my-1
         ...
         do i=pb,pe
            do k=0,mz1
               ...
               do j=0,my-1
                 ...
                 ten12(j,k,i)=xalp(i-1)*nxwk(j,k,i)+
     .                       xbet(k)*nzwk(j,k,i)
                 ten13(j,k,i)=xalp(i-1)*mxwk(j,k,i)+
     .                       xbet(k)*mzwk(j,k,i)
!                  !aux2
               enddo
            enddo
...
!we need the derivative on y of aux2
         do i=pb,pe
              call deryr2(ten12(0,0,i),nywk(0,0,i),mz)
              call deryr2(ten13(0,0,i),ten23(0,0,i),mz)
         enddo 
...
!                  !RHS(phi)
                 ten13(j,k,i)=mywk(j,k,i)-nywk(j,k,i)-ire*ten23(j,k,i)
               enddo
            enddo
         enddo
...
!******************************************************************!
      subroutine foup2non(up1,up2,up3,ten11,ten22,ten33,
     .           rhst,ten12,ten13,ten23,
     .           rhsz,drho,lapz,RHSZp,
     .           chwk,sp,myid,rkstep)
      use MPI_GROUPS
      use point
      use diag
...
c     ---------------------- Variables ----------------------
     ...
     .          ten13,ten23,rhst,rhsz,drho,lapz,chwk
      ...
      real(doubleprec),dimension(0:mgalx-1,lb:le):: RHSZp
...
! are still lines in fourier space x (phys z).
      call phys2non(up1,up2,up3,ten11,ten22,ten33,
...
!                                                                     !
!*********************************************************************!
      subroutine phys2non(up1,up2,up3,ten11,ten22,ten33,
     .           rhst,ten12,ten13,ten23,rhsz,drho,lapz,
     .           RHSZp,myid,rkstep)
      use point
      use tem
...
c     --------------------------- Variables -------------------      
      ...
      real(doubleprec)  cflx,cfly,cflz,hxalp,hzbet,hyy,cfl0,reigmx1,aa,
...
!debugging
      ...
      real(doubleprec):: onedum,zerodum,outdum
...
c        Move everything to pPP, line by line 
      ...
      dumc = 0d0
      Tmax=0d0
      !if (diffcoef.le.0.0) then
      !    write(*,*) "diffcoef calculated wrongly!!"
      !endif
...
!         !Calculate new rhs(H),rhs(Z)
         do i=0,mgalx-1
             !calcrhsH,Z
             call calcrhshz(outdum,RHSZp(i,j),
     .      tmp1wk8(i),tmp2wk8(i),tmp3wk8(i),
     .      onedum,zerodum,zerodum,zerodum,
...
!*******************************************************
     ...
     .           lapH,lapZ)
      ...
      implicit none
      include "ctes3D" 
      real(doubleprec),intent(in)::u,v,w,Temp,dHx,dHy,dHz,Z,Zx,Zy,Zz,
     .                          lapH,lapZ
      real(doubleprec),intent(out) :: rhsh,rhsz
      real(doubleprec)::dTx,dTy,dTz,dZbx,dZby,dZbz,lapZb
...
!*******************************************************
      ...
      implicit none
      include "ctes3D"
      real(doubleprec),intent(in)  :: Z,dHdxi,dZdxi
      real(doubleprec),intent(out) :: dTdxi
