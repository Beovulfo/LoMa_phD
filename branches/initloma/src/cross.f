...
c ========================================================================
      ...
      !save istep into ext1
         ...
         if (mod(istep-1,nhist).eq.(nhist-ntimes).and.nstart.ne.0) then
             istati = 1 !accumulating stats...
         endif
         if (mod(istep-1,nhist).eq.0.and.istep.ne.1) then 
             istati = 0 !not more accumulation
             call writestats(myid,istep) !write to file STATS
             nacum = 0
            !clean stats after writing history files (out of rkstep
            !loop)
         endif

         if (mod(istep-1,nhist).eq.0) ihist=1 !write hist (.cf)
...
!======================================================!
     ...
     .       ten23(0,0,i),ten13(0,0,i),ten12(0,0,i),chwk,uner,i)
          enddo
...
!    1.2) Update v00
             do j=0,my1 
                v00wk(j)=-rf0v(j)/(dtgamma+dtxi) !Euler Implicit
             enddo
         endif
...
324            format(14(d22.14))
               write(29,324)  time,dm,enerdis, ener,massu,massw,
...
! AAF June 2014
!--------------------------------------------------------!
      subroutine addstats1(u1c,u2c,u3c,Tc,chwkc,plane)
...
!              INTENSITIES
               ...
               up(j) = up(j) + u1c(j,k)*conjg(u1c(j,k))
              
               ener(2) = u2c(j,k)*conjg(u2c(j,k))
               vp(j) = vp(j) + u2c(j,k)*conjg(u2c(j,k))

               ener(3) = u3c(j,k)*conjg(u3c(j,k))
               wp(j) = wp(j) + u3c(j,k)*conjg(u3c(j,k))

               ener(4) = real(u1c(j,k)*conjg(u2c(j,k)))
               uvr(j)= uvr(j)+real(u1c(j,k))*conjg(u2c(j,k))

               ener(5) = real(u1c(j,k)*conjg(u3c(j,k)))
               uwr(j)= uwr(j)+real(u1c(j,k))*conjg(u3c(j,k))

               ener(6) = real(u3c(j,k)*conjg(u2c(j,k)))
               vwr(j)= vwr(j)+real(u2c(j,k))*conjg(u3c(j,k))

               ener(7) = o1c(j,k)*conjg(o1c(j,k))
               w1p(j) = w1p(j) + o1c(j,k)*conjg(o1c(j,k))

               ener(8) = o2c(j,k)*conjg(o2c(j,k))
               w2p(j) = w2p(j) + o2c(j,k)*conjg(o2c(j,k))

               ener(9) = o3c(j,k)*conjg(o3c(j,k))
               w3p(j) = w3p(j) + o3c(j,k)*conjg(o3c(j,k))
...
!--------------------------------------------------------!
! addstats3
! AAF june 2014 -> this add vorticity to stats
!--------------------------------------------------------!
      subroutine addstats3(u1c,u2c,u3c,chwkc,plane)
...
! AAF June 2014
!....................................................................!
      subroutine writestats(myid,istep)
...
!-------------Variables----------------------------!
      integer myid,i,j,k,ierr,istat(MPI_STATUS_SIZE)
      real*4 fac
      character*84 fnamesta
      character*3 ext1
