...
%
%
filename='mlpantano03.032'
rfactor=5.3;%Scaled with dm(tau) from file
...
%vor=reshape(wk2(:,1,:,:),mx/2,2*my,mz);
%phi=reshape(wk2(:,2,:,:),mx/2,2*my,mz);
%for (j=1:my)
% vor_real(:,j,:)=vor(:,2*j-1,:);
% vor_imag(:,j,:)=vor(:,2*j,:);
%end
%for (j=1:my)
% phi_real(:,j,:)=phi(:,2*j-1,:);
% phi_imag(:,j,:)=phi(:,2*j,:);
%end
%vor2=complex(vor_real,vor_imag);
%phi2=complex(phi_real,phi_imag);
...
%wk1==>u00, w00, odd/even
u00=interp1(y/rfactor,u00,y,'spline',0)
for (j=1:mye)
