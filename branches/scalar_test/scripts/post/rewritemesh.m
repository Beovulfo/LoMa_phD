...
%
%
filename='finpmlpantano09.dat'
finp=fopen(filename,'r','l');
rfactor=1;
%new mesh
ymax=172/4; %half heigth of the box
y2=create_mesh1D(ymax,'mixl', 0.3,mye); 
%----------------------Record 1----------------------------%
%Buffer size    
...
for i=1:nplanes
    ...
    for k=1:mz
%scale and save vor 
      wk2(1,indreal  ,k)=interp1(y,wk2(1,indreal  ,k),y2,'linear',0);
      wk2(1,indreal+1,k)=interp1(y,wk2(1,indreal+1,k),y2,'linear',0);
%write PHI
      wk2(2,indreal  ,k)=interp1(y,wk2(2,indreal  ,k),y2,'linear',0);
      wk2(2,indreal+1,k)=interp1(y,wk2(2,indreal+1,k),y2,'linear',0);
    end
%----------------------------------------------------!
%Writing
