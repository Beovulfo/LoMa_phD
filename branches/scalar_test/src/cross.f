...
c ------------------- Variables ----------------------------------------
     ...
     .          scalrhs(0:my-1,0:mgalz-1,pb:pe)
...
!aaf calculate derivative of scal: scal2=d(scal1)/dy -- F-F
            call deryr2(scal1(0,0,i),scal2(0,0,i),mz)
         enddo
c---------Calculate dc/dx,dc/dz------------------------F-F
         do i=pb,pe
            do k=0,mz1
               do j=0,my-1
                  !dc/dz=i kz c
                  scal3(j,k,i)=xbet(k)*scal1(j,k,i)
                  !dc/dx=i kx c
                  scal1(j,k,i)=xalp(i-1)*scal1(j,k,i)
               enddo
            enddo
        enddo
...
!compute new non linear term, update phi, saves the nonlinear term.
     ...
     .             ire, dtgamma,dtxi)
!scalar rkstepexp
              call rkstepexp(scal(0,0,i),scalrhs(0,0,i),scal1(0,0,i),
     .             mz,i,scadiff,dtgamma,dtxi)
...
324            format(17(d22.14))
               write(29,324) time,scal(1,0,1),reynota,dm,ener,
...
325            format(i5,12(d14.6))
               write(*,325)istep,time,scal(1,0,1),reynota,dm,Deltat,
...
!scalar
      call localyz2zy(scal1,scal1,chwk)         
      call localyz2zy(scal2,scal2,chwk)         
      call localyz2zy(scal3,scal3,chwk)         
...
!scalar
         call fourz(scal1(0,0,i),1)  ! dscal/dx
         call fourz(scal2(0,0,i),1)  ! dscal/dy
         call fourz(scal3(0,0,i),1)  ! dscal/dz
      enddo
...
!scalar
      call chpl2ln(scal1,scal1,chwk,myid)
      call chpl2ln(scal2,scal2,chwk,myid)
      call chpl2ln(scal3,scal3,chwk,myid)
...
!scalar
      call chln2pl(scal1,scal1,chwk,myid)   
...
!scalar
         call fourz(scal1(0,0,i),-1)  !NLscal
...
!scalar
      call localzy2yz(scal1,scal1,chwk)   !NLscal out
...
c     --------------------------- Variables -------------------      
     ...
     .        scal1(0:mx-1,lb:le),scal2(0:mx-1,lb:le),
     .        scal3(0:mx-1,lb:le)
...
!scalars
         call fourx(scal1wk,scal1wk8,1)    ! omega_3
         call fourx(scal2wk,scal2wk8,1)    ! omega_3
         call fourx(scal3wk,scal3wk8,1)    ! omega_3
