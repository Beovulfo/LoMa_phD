***********************************************************************!
!      INTERP FOR REACTML/LOMAHZ -> new y-mesh
!-----------------------------------------------------------------------!
!       a. almagro[aaf] (2016-)
!***********************************************************************!
      Program main
...
!--------------- initializes commons and things 
      ...
      write(*,*) 'INITCR passed ...', myid
      
!--------------  allocates spectra and buffers
      mym = max(my,mye)
      nwkasize = 2*2*4*mmp*mym*mz+1000
       !complex single prec
      ...
      if (myid.lt.numerop) then     ! Comp. Proc.
         ...
         allocate(vor2(2*mye*mz*(pe-pb+1)))
         allocate(phi2(2*mye*mz*(pe-pb+1)))
         allocate(psi2(2*mye*mz*(pe-pb+1)))
         allocate(scal2(2*mye*mz*(pe-pb+1)))
         !allocate(mfz2(2*mye*mz*(pe-pb+1)))
      ...
      !index for SAVE PROCS
       ...
       ivor2  = iscal + 2*mmp*mye*mz
       iphi2  = ivor2 + 2*mmp*mye*mz
       ipsi2  = iphi2 + 2*mmp*mye*mz
       iscal2  = ipsi2 + 2*mmp*mye*mz
      ...
      if (myid.lt.numerop) then
         ...
         enddo
          ...
          call getfil(vor,phi,psi,scal,u00,v00,w00,wk,myid)!pending
...
!      ---------------  read from file and distribute
      ...
      if (myid.eq.master) then     
         ...
         read(iinp) time,Ree,alpe,bete,a0e,mxe,mye0,mze,
     .               (y(j),fmape(j),j=1,my),(wk1(j),j=1,3*my)

         write(*,*) 
         write(*,*) 'reading input file ...' 
         write(*,*) 'time=',time,'Ree=',Ree,'alpe=',alpe,
     .   'bete=',bete,'mxe=',mxe,'mye0=',mye0,'mze =', mze
     .   ,"my=",my
         write(*,*)
         ...
         !ntotr=4*mye*mze
         ntotr=4*2*mye0*mze !vor phi psi scal mfz x2
         if (myid.eq.0) then
           if (my.ne.mye0) then
               write(*,*) "MY is not the same as MYE" 
               stop
           endif
         endif
!     ------------- 00 modes ------------------
...
!     ------------ 00 modes only for master 
     ...
     &                iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(mye0,1,MPI_INTEGER,iproc,
     &                iproc,MPI_COMM_WORLD,ierr)
...
!         ------------  other modes,  distribute to slaves --
     ...
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(mye0,1,MPI_INTEGER,master,
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
...
!   !--------------------- variables -------------------------------------
      ...
      integer i,j,iproc,k,ifor,blocks,elem,stride
...
167      read(19,'(a)') text
     ...
     &                 iproc,MPI_COMM_WORLD,ierr)
         ...
         call MPI_RECV(mye,1,MPI_INTEGER,0,
     &                 MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
      endif
...
c ---------------  modes values for FOURIER
     ...
     .               (y(j),fmap(j),j=1,my)
         ...
         close(iinp)
!        READ NEW MESH
         write(*,*) "Reading new y-mesh..."
         open(ispf,file=filstt,status='old')
         read(ispf,*) (ye(j),j=1,mye)
         rewind(ispf)
         close(ispf)
         do iproc=1,numtot-1
            call MPI_SEND(y,my,MPI_REAL8,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
            call MPI_SEND(ye,mye,MPI_REAL8,iproc,
     &                 iproc,MPI_COMM_WORLD,ierr)
     ...
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
         call MPI_RECV(ye,mye,MPI_REAL8,0,
     &                MPI_ANY_TAG,MPI_COMM_WORLD,istat,ierr)
...
!          adapted   shc 5/09/04
!***********************************************************************!
      subroutine assign(work,vor,phi,psi,scal,
     .                  myid,planex)
      use input
      implicit none
      include "ctes3D"

      integer   mzm,klen,kini1,kini2,j,k,k1,k2
      real(realprec)    vor(2*my,mz),phi(2*my,mz)
      real(realprec)    psi(2*my,mz),scal(2*my,mz)
      real(4)    work(4,2*mye0,mze)
      real(realprec)    zero
      integer planex
      integer   myid
      integer flag
