...
Pr    = 1;       % ratio between diffusivity and viscosity
r     = 1;       % density ratio
T     = 20;      % total time
CFL   = 0.1;     % CFL
Lx    = 2*pi;    % x length
...
%----------------------------------------------%
lam_v = 2./Re.*(1./dx.^2 + 1./dy.^2 + 1./dz.^2); 
...
%   Taylor 2D initial conditions (test case): 
   ...
   %   continuity implies they have to satisfy Aa+Bb+Cc=0; 
       ...
       tem = tem + (r-1)./(r+1).*cos(2*pi*zz./Lz); 
...
%      save and print:
    ...
    % Explicit Euler for ome, phi, tem and mx00_hat and mz00_hat: 
    phi_hat = phi_hat + dt.*RHSphi_hat; 
    ome_hat = ome_hat + dt.*RHSome_hat; 
    tem_hat = tem_hat + dt.*RHStem_hat; 
    mx00_hat = mx00_hat + dt.*mx_hat(:,1,1); 
    mz00_hat = mz00_hat + dt.*mz_hat(:,1,1); 


    % Implicit Euler for psi: 
    % qqq = real( ifftn(RHStem_hat) ); % STIL IS, check a few lines earlier
    kkk = real( ifftn(tem_hat) ); 
    kkk = qqq./(kkk-dt.*qqq)./kkk; 
    RHSpsi_hat=fftn(kkk,ndim); RHSpsi_hat = RHSpsi_hat.*dealias; 
   
    psi_hat = RHSpsi_hat./(-k2o);   
